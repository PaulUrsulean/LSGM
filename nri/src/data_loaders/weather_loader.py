...
class WeatherDataset(Dataset):
    ...
    def __init__(self, n_samples, n_nodes, n_timesteps, features, filename=None, dataset_path=None, force_new=False,
                 discard=False, from_partial=False, normalize=False, normalize_params=None, dset=None,
                 existing_config=None, existing_indices=None, threshold=1):
        """
        ...
        Args:
            ...
            existing_indices(list[int]): List of integers indexing into existing_config, with their indices
                corresponding to the sample they belong to.
        ...
        assert self.dset.shape == (self.n_samples, self.n_nodes, self.n_timesteps,
                                   len(self.features)), "Dataset dimensions do not match specifications"

        if self.normalize:
            if self.__normalize_params is None:
                self.__normalize_params = tuple((self.dset[:, :, :, feature].mean(),
                                                 self.dset[:, :, :, feature].std()) for feature in
                                                range(dset[:].shape[-1]))
    ...
    def __getitem__(self, idx):
        ...
        if self.normalize and self.__normalize_params is None:
            ...
            self.__normalize_params = tuple(
                (self.dset[:, :, :, feature].mean(), self.dset[:, :, :, feature].std()) for feature in range(shape[-1]))
            self.__normalize_dataset()
    ...
    def generate_dset(self, filename, dataset_path, force_new, discard, from_partial, threshold):
        ...
        if filename is None:
            ...
            for f in npy_files:
                ...
                if int(spl[0]) >= self.n_samples and int(spl[1]) == self.n_nodes and int(
                        spl[2]) == self.n_timesteps and int(spl[3]) == len(self.features):
        ...
        if already_exists and not force_new and not from_partial:
            ...
            assert self.dset.shape == (self.n_samples, self.n_nodes, self.n_timesteps, len(
                self.features)), "Given file name contains dataset of a different shape than specified in parameters."
            ...
            self.save_file_name = join(data_dir, clean_fname) \
                if filename is not None else join(data_dir,
            ...
            self.dset, self.configurations, self.config_indices = \
                self.sample_configurations(data, self.n_samples,
                                           self.n_nodes,
                                           self.n_timesteps,
                                           ...
                                           existing_config,
                                           existing_indices,
                                           partial_save_freq=partial_save_freq)
    ...
    def train_valid_test_split(cls, dset, features, spl=[80, 10, 10], normalize=False, export=False):
        ...
        train_set = cls(n_train, n_nodes, n_timesteps, features, dset=train_dset, existing_config=all_configs,
                        existing_indices=train_config_indices, normalize=normalize)
        ...
        test_set = cls(
            n_samples - (n_train + n_valid), n_nodes, n_timesteps, features, dset=test_dset,
            existing_config=all_configs,
    ...
    def remove_deep(src, matching_dates):
        ...
        src_dates = src['date']
        trg_dates = matching_dates['date'].drop_duplicates() \
            if isinstance(matching_dates, pd.DataFrame) else matching_dates.drop_duplicates()
    ...
    def sample_configurations(self, df, n_samples, n_nodes, n_timesteps, features, threshold, existing_dset=[],
                              existing_config=[], existing_indices=[], partial_save_freq=1000):
        """
        ...
        partial_save_thresh = 0 if existing_dset == [] else len(existing_dset) - (
                len(existing_dset) % partial_save_freq)
        ...
        while current_samples < n_samples:
            ...
            if len(df_conf['station_id'].unique()) == n_nodes and len(df_conf) >= sample_df_size:
                ...
                # Group values into quasi-consecutive clusters (up to a threshold specified in function
                # param "threshold"). Here we do not cluster unique dates, but rows
                # in the dataframe sorted by date, for which there are n_nodes for
                ...
                for cluster in clusters_indices:
                    ...
                    samples_to_use = samples_in_cluster if current_samples + samples_in_cluster <= n_samples else \
                    ...
                    # Sample simulations randomly if we need to lower the number due to reaching the
                    # required amount of samples in the dataset
                    if samples_to_use < samples_in_cluster:
                        sampled_simulations = np.random.choice(np.arange(samples_in_cluster), size=samples_to_use,
                                                               replace=False)
                    ...
                    # Increase threshold by the frequency when it is passed to simulate
                    # modulo behavior for nonconsecutive values
                    if current_samples >= partial_save_thresh and partial_save_freq != 0:
                        ...
                        print("{} samples progress saved in partial "
                              "file {}".format(len(dataset), self.save_file_name + "_partial.npy"))
